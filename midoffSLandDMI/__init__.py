...
line_length = 1.4  # 30pix is 42 min of arc on 57cm distance
...
basenameLS = os.path.basename('ls.png')
img.save(os.path.join(to_dir, basenameLS), quality=100)

# other images prepare
for j in [1, -1]: # Cross or uncross disparity
    for k in variation:
        img = Image.new("RGB", (sz, sz), (lb, lb, lb))
        draw = ImageDraw.Draw(img)

        # Fill the targets
        draw.rectangle((int(sz / 2) - int(f / 2) + disparity*j, int(sz / 2) + int(ll / 2),
                        int(sz / 2) + int(f / 2) + disparity*j, int(sz / 2) - int(ll / 2)),
                       fill=(0, 0, 0), outline=None)

        draw.rectangle((int(sz / 2) - int(f / 2) + disparity*j, int(sz / 2) + int(ll / 2)*k,
                        int(sz / 2) + int(f / 2) + disparity*j, int(sz / 2) - int(ll / 2)*k),
        ...
        # Write images
        basename = os.path.basename(str(j) + 'ds' + str(k) + '.png')
    ...
    basenameLS = os.path.basename(str(j) + 'ds0.png')
    img.save(os.path.join(to_dir, basenameLS), quality=100)
